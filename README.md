...
RAG를 붙이면서 설계를 이렇게 잡았다. 우선 내가 다루는 자료를 잘게 나눠 임베딩하고, ChromaDB 같은 벡터 스토어에 메타데이터와 함께 넣었다. 단순 임베딩 검색만으로는 놓치는 문장이 있어 BM25 키워드 검색을 앞단에 얹고, 임베딩 기반 재순위로 상위 후보를 재정렬하는 하이브리드를 썼다. 파편화로 맥락이 끊기는 걸 줄이려고 청크 크기와 오버랩을 여러 조합으로 테스트했다. 프롬프트는 인물 페르소나형 대화가 필요할 땐 캐릭터 메타데이터를 주었다. UI는 Streamlit로 빠르게 구현했다.
